...
Future<List<namapegawai>> fetchMhss(http.Client client) async {
  final response =
      await client.get('https://habibiahmad.000webhostapp.com/namapegawai.php');

  // Use the compute function to run parseMhss in a separate isolate.
  return compute(parseMhss, response.body);
}

// A function that converts a response body into a List<namapegawai>.
List<namapegawai> parseMhss(String responseBody) {
  final parsed = jsonDecode(responseBody).cast<Map<String, dynamic>>();

  return parsed.map<namapegawai>((json) => namapegawai.fromJson(json)).toList();
}
...
class namapegawai {
  ...
  namapegawai({this.nip, this.nama_pegawai, this.departemen, this.jabatan, this.pendidikan_terakhir});

  factory namapegawai.fromJson(Map<String, dynamic> json) {
    return namapegawai(
...
class MyHomePage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder<List<namapegawai>>(
...
class MhssList extends StatelessWidget {
  final List<namapegawai> MhsData;
